...
StreamT& PrintTabs(StreamT& out, uint depth) {
...
std::ostream& PrintTabs(std::ostream& out, uint depth) {
...
template<class StreamT>
StreamT& PrintString(StreamT& out, const String& text) {
...
template<class StreamT>
void PrintAsciiChar(StreamT& out, char c) {
	assert(out.capacity() < 1024 * 1024 * 500); // arbitrary
	assert(out.size() < 1024 * 1024 * 500);
	switch (c) {
		case '\\':  out << "\\\\";break;
...
template<class StreamT>
void PrintText(StreamT& out, const String& text)
{
	String::const_utf8iterator end = text.utf8end();
	String::const_utf8iterator i   = text.utf8begin();
...
void PrintSubNodesIDs(Clob& out, const Node& node) {
...
void PrintSubNodesLinks(Clob& out, const Node::Map& rules, const Node& node, const String& source, bool inverseColor) {
		...
		case Node::asAND:
		{
			...
			else
			{
				...
				for (uint i = 0; i != (uint) children.size(); ++i)
				{
					...
					helpers[hi] << "inline bool __helper" << (from + i) << "(Datasource& ctx)\n";
					helpers[hi] << "{\n";
					helpers[hi] << "\tTRACE(\"    :: entering helper " << (from + i) << "\");\n";
					children[i].exportCPP(helpers[hi], rules, helpers, datatext, 1, true, sp);
