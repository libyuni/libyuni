...
if (WIN32)
	ynmessage("unzip: ${DevPackSourceZIP}")
endif()
...
macro(DEVPACK_IMPORT dpname dpversion dprelease dpos dparch dpcompiler dptarget)
	...
	if (NOT DevPackReady)
		...
		ynmessage("   ${DevPackTitle}: Missing")
		...
		if (NOT DevPackReceiptReady)
			ynmessage("    . Downloading ${DevPackURL}")
			...
			if (NOT DevPackReceiptReady)
				ynmessage("    . !! The download has failed")
		...
		if (DevPackReceiptReady)
			# Execute `unzip`
			ynmessage("    . Extracting the receipt file")
		...
		ynmessage("   ${DevPackTitle}")
	endif()

	if (NOT DevPackReady)
		ynmessage()
		ynmessage("[!!] The installation of the DevPack `${dpname}` has failed.")
		ynmessage("     You can download the DevPack from this url :")
		ynmessage("      `${DevPackURL}`")
		ynmessage("     And put it into the `receipts` folder :")
		ynmessage("      `${DevPackReceiptsFolder}`")
		ynmessage()
		ynmessage("[!!] If the devpack is already available in the receipts folder and you still have")
		ynmessage("     issues, please manually remove it and try again.")
		ynmessage("     Please visit `${DevPackRepositoryURL}` for all devpacks.")
		ynmessage()
